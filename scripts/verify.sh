...
python3 scripts/unbound_local_check.py
...
CONTAINER_NAME=${CONTAINER_NAME:-Yoishizuku-bot}
status=$(docker inspect "$CONTAINER_NAME" --format '{{.State.Status}}' 2>/dev/null || true)
restarts=$(docker inspect "$CONTAINER_NAME" --format '{{.RestartCount}}' 2>/dev/null || true)
[ "$status" = "running" ]
[ "$restarts" = "0" ]

docker exec -e PYTHONPYCACHEPREFIX=/tmp/pycache "$CONTAINER_NAME" \
