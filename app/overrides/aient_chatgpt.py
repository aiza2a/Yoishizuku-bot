...
def _messages_have_tool_roles(messages):
...
def _friendly_tool_name(name_or_id):
    raw = str(name_or_id or "tool")
    base = re.sub(r"(_single|_tool_call|_call)$", "", raw)
    mapping = {
        "get_time": "当前时间",
        "get_search_results": "网络搜索",
        "generate_image": "文生图",
        "run_python_script": "代码执行",
        "get_url_content": "网址总结",
        "download_read_arxiv_pdf": "论文",
    }
    return mapping.get(base, base)


def _convert_tool_messages_for_upstream(messages):
    """Convert OpenAI tool-call transcripts into plain chat messages.

    Some providers accept tools on first request but reject follow-up
    assistant.tool_calls / role=tool messages. Rewrite for compatibility
    without exposing internal tool IDs to the user.
    """
    if not isinstance(messages, list):
        return messages
        ...
        if role == "assistant" and msg.get("tool_calls"):
            ...
            if not content:
                ...
                for tc in msg.get("tool_calls") or []:
                    try:
                        names.append(((tc or {}).get("function") or {}).get("name") or "")
                    except Exception:
                        pass
                friendly_names = [_friendly_tool_name(n) for n in names if n]
                friendly = "、".join(friendly_names) if friendly_names else "工具"
                content = "正在使用" + friendly + "获取信息。"
            converted.append({"role": "assistant", "content": content})
            continue

        if role == "tool":
            tool_id = msg.get("tool_call_id") or "tool"
            friendly = _friendly_tool_name(tool_id)
