...
def get_doc_from_url(url):
    parsed_name = Path(urllib.parse.unquote(urllib.parse.urlsplit(url).path)).name
    suffix = Path(parsed_name).suffix[:12]
    response = requests.get(url, stream=True, timeout=(15, 120))
    response.raise_for_status()
    fd, filename = tempfile.mkstemp(prefix='gptbot-doc-', suffix=suffix, dir='/tmp')
    try:
...
async def Document_extract(docurl, docpath=None, engine_type = None):
    ...
    if docpath and docurl and "paper.pdf" != docpath:
        docpath = get_doc_from_url(docurl)
        filename = docpath
...
def check_json(json_data):
    while True:
        try:
            ...
            logging.warning("Tool JSON parse error: %s; chars=%s", e, len(str(json_data)))
    ...
    if isinstance(functions_list, str):
        try:
            ...
            if not isinstance(functions_list, list):
                logging.warning("Extracted tool call is not a list; type=%s", type(functions_list).__name__)
        except json.JSONDecodeError as e:
            logging.warning("Tool call JSON parse failed: %s", e)
